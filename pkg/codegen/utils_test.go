...
func TestToCamelCaseWithInitialisms(t *testing.T) {
	...
	for i := range tests {
		tt := tests[i]
		t.Run(tt.str, func(t *testing.T) {
			require.Equal(t, tt.want, defaultInitialisms.ToCamelCaseWithInitialisms(tt.str))
		})
	}
}
...
func Test_replaceInitialism(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(t, tt.want, defaultInitialisms.replaceInitialism(tt.args.s), "replaceInitialism(%v)", tt.args.s)
		})
	}
}
