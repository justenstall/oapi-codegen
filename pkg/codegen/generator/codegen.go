...
func (state *State) GenerateTypeDefinitions(t *template.Template, swagger *openapi3.T, ops []OperationDefinition, excludeSchemas []string) (string, error) {
	...
	enumsOut, err := state.GenerateEnums(t, enumTypes)
	if err != nil {
...
func (state *State) GenerateTypesForSchemas(t *template.Template, schemas map[string]*openapi3.SchemaRef, excludeSchemas []string) ([]TypeDefinition, error) {
	...
	for _, schemaName := range SortedSchemaKeys(schemas) {
		...
		goTypeName, err := renameSchema(schemaName, schemaRef, state.NameNormalizer)
		if err != nil {
...
func (state *State) GenerateTypesForParameters(t *template.Template, params map[string]*openapi3.ParameterRef) ([]TypeDefinition, error) {
	...
	for _, paramName := range SortedMapKeys(params) {
		...
		goTypeName, err := renameParameter(paramName, paramOrRef, state.NameNormalizer)
		if err != nil {
		...
		if paramOrRef.Ref != "" {
			...
			typeDef.TypeName = SchemaNameToTypeName(refType, state.NameNormalizer)
		}
...
func (state *State) GenerateTypesForResponses(t *template.Template, responses openapi3.ResponseBodies) ([]TypeDefinition, error) {
	...
	for _, responseName := range SortedMapKeys(responses) {
		...
		for _, mediaType := range SortedMapKeys {
			...
			goTypeName, err := renameResponse(responseName, responseOrRef, state.NameNormalizer)
			if err != nil {
...
func (state *State) GenerateTypesForRequestBodies(t *template.Template, bodies map[string]*openapi3.RequestBodyRef) ([]TypeDefinition, error) {
	...
	for _, requestBodyName := range SortedMapKeys(bodies) {
		...
		for mediaType, body := range response.Content {
			...
			goTypeName, err := renameRequestBody(requestBodyName, requestBodyRef, state.NameNormalizer)
			if err != nil {
...
func (state *State) SetSpec(spec *openapi3.T) {
	state.Spec = spec
}
...
func (state *State) SetOptions(opts Configuration) error {
	...
	var err error
	state.NameNormalizer, err = getNameNormalizer(opts)
	if err != nil {
		return err
	}
	return nil
...
func getNameNormalizer(opts Configuration) (NameNormalizer, error) {
	switch {
	// Set with function
	case opts.OutputOptions.NameNormalizerFunction != nil:
		return opts.OutputOptions.NameNormalizerFunction, nil
	// Set by name
