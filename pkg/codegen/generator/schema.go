...
func (state *State) goFieldName(p Property) string {
	...
	return SchemaNameToTypeName(goFieldName, state.NameNormalizer)
}
...
func (d *Discriminator) PropertyName(nameNormalizer NameNormalizer) string {
	return SchemaNameToTypeName(d.Property, nameNormalizer)
}
...
func (state *State) GenerateGoSchema(sref *openapi3.SchemaRef, path []string) (Schema, error) {
	...
	if IsGoTypeReference(sref.Ref) {
		...
		refType, err := state.RefPathToGoType(sref.Ref)
		if err != nil {
	...
	if t.Slice() == nil || t.Is("object") {
		...
		if len(schema.Properties) == 0 && !SchemaHasAdditionalProperties(schema) && schema.AnyOf == nil && schema.OneOf == nil {
			...
			if schema.AdditionalProperties.Schema != nil {
				...
				if additionalSchema.HasAdditionalProperties || len(additionalSchema.UnionElements) != 0 {
					...
					// to get to the type.
					typeName := PathToTypeName(append(path, "AdditionalProperties"), state.NameNormalizer)
			...
			if !state.Options.Compatibility.DisableFlattenAdditionalProperties &&
			...
			for _, pName := range SortedSchemaKeys(schema.Properties) {
				...
				if (pSchema.HasAdditionalProperties || len(pSchema.UnionElements) != 0) && pSchema.RefType == "" {
					...
					// for them, which will be based on the field names we followed
					// to get to the type.
					typeName := PathToTypeName(propertyPath, state.NameNormalizer)
		...
		err := state.oapiSchemaToGoType(schema, path, &outSchema)
		...
		for k, v := range sanitizedValues {
			...
			if state.Options.Compatibility.OldEnumConflicts {
				outSchema.EnumValues[SchemaNameToTypeName(PathToTypeName(append(path, enumName), state.NameNormalizer), state.NameNormalizer)] = v
			} else {
				outSchema.EnumValues[SchemaNameToTypeName(k, state.NameNormalizer)] = v
			}
		}
		if len(path) > 1 { // handle additional type only on non-toplevel types
			...
			if extension, ok := schema.Extensions[extGoTypeName]; ok {
				...
				typeName = SchemaNameToTypeName(PathToTypeName(path, state.NameNormalizer), state.NameNormalizer)
			}
...
func (state *State) oapiSchemaToGoType(schema *openapi3.Schema, path []string, outSchema *Schema) error {
	f := schema.Format
	t := schema.Type
	...
	if t.Is("array") {
		...
		if (arrayType.HasAdditionalProperties || len(arrayType.UnionElements) != 0) && arrayType.RefType == "" {
			...
			// for them, which will be based on the field names we followed
			// to get to the type.
			typeName := PathToTypeName(append(path, "Item"), state.NameNormalizer)
...
// GenFieldsFromProperties produce corresponding field names with JSON annotations,
// given a list of schema descriptors
func (state *State) GenFieldsFromProperties(props []Property) []string {
	var fields []string
	for i, p := range props {
		field := ""
		...
		if p.Nullable && state.Options.OutputOptions.NullableType {
...
func (state *State) generateUnion(outSchema *Schema, elements openapi3.SchemaRefs, discriminator *openapi3.Discriminator, path []string) error {
	...
	for i, element := range elements {
		...
		if element.Ref == "" {
			elementName := SchemaNameToTypeName(PathToTypeName(elementPath, state.NameNormalizer), state.NameNormalizer)
