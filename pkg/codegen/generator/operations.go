...
func (pd ParameterDefinition) GoVariableName(nameNormalizer NameNormalizer) string {
	name := LowercaseFirstCharacters(pd.GoName(nameNormalizer))
	if IsGoKeyword(name) {
...
func (pd ParameterDefinition) GoName(nameNormalizer NameNormalizer) string {
	goName := pd.ParamName
	...
	return SchemaNameToTypeName(goName, nameNormalizer)
}
...
func (r ResponseDefinition) GoName(nameNormalizer NameNormalizer) string {
	return SchemaNameToTypeName(r.StatusCode, nameNormalizer)
}
...
// TypeDef returns the Go type definition for a request body
func (r ResponseContentDefinition) TypeDef(opID string, statusCode int, nameNormalizer NameNormalizer) *TypeDefinition {
	return &TypeDefinition{
		TypeName: fmt.Sprintf("%s%v%sResponse", opID, statusCode, r.NameTagOrContentType(nameNormalizer)),
		Schema:   r.Schema,
	}
}
...
func (r ResponseContentDefinition) NameTagOrContentType(nameNormalizer NameNormalizer) string {
	if r.NameTag != "" {
		return r.NameTag
	}
	return SchemaNameToTypeName(r.ContentType, nameNormalizer)
}
...
func (state *State) OperationDefinitions(swagger *openapi3.T, initialismOverrides bool) ([]OperationDefinition, error) {
	...
	for _, requestPath := range SortedMapKeys(swagger.Paths.Map()) {
		...
		for _, opName := range SortedMapKeys(pathOps) {
			...
			state.ensureExternalRefsInParameterDefinitions(&allParams, pathItem.Ref)
			...
			state.ensureExternalRefsInResponseDefinitions(&responseDefinitions, pathItem.Ref)
...
func (state *State) GenerateResponseDefinitions(operationID string, responses map[string]*openapi3.ResponseRef) ([]ResponseDefinition, error) {
	...
	for _, statusCode := range SortedMapKeys(responses) {
		...
		for _, headerName := range SortedMapKeys(response.Headers) {
			...
			headerDefinition := ResponseHeaderDefinition{Name: headerName, GoName: SchemaNameToTypeName(headerName, state.NameNormalizer), Schema: contentSchema}
...
func (state *State) GenerateParamsTypes(op OperationDefinition) []TypeDefinition {
	...
	for _, param := range objectParams {
		...
		if pSchema.HasAdditionalProperties {
			propRefName := strings.Join([]string{typeName, param.GoName(state.NameNormalizer)}, "_")
