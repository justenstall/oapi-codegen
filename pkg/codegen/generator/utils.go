...
func (state *State) refPathToGoTypeSelf(refPath string, local bool) (string, error) {
	...
	// the spec.
	name, err := findSchemaNameByRefPath(refPath, state.Spec, state.NameNormalizer)
	if err != nil {
	...
	return SchemaNameToTypeName(lastPart, state.NameNormalizer), nil
}
...
func (state *State) SanitizeEnumNames(enumNames, enumValues []string) map[string]string {
	...
	for _, p := range deDup {
		n, v := p[0], p[1]
		sanitized := SanitizeGoIdentity(SchemaNameToTypeName(n, state.NameNormalizer))
...
// returned.
func renameSchema(schemaName string, schemaRef *openapi3.SchemaRef, nameNormalizer NameNormalizer) (string, error) {
	// References will not change type names.
	if schemaRef.Ref != "" {
		return SchemaNameToTypeName(schemaName, nameNormalizer), nil
	}
...
// account
func renameParameter(parameterName string, parameterRef *openapi3.ParameterRef, nameNormalizer NameNormalizer) (string, error) {
	if parameterRef.Ref != "" {
		return SchemaNameToTypeName(parameterName, nameNormalizer), nil
	}
...
// account
func renameResponse(responseName string, responseRef *openapi3.ResponseRef, nameNormalizer NameNormalizer) (string, error) {
	if responseRef.Ref != "" {
		return SchemaNameToTypeName(responseName, nameNormalizer), nil
	}
...
// account
func renameRequestBody(requestBodyName string, requestBodyRef *openapi3.RequestBodyRef, nameNormalizer NameNormalizer) (string, error) {
	if requestBodyRef.Ref != "" {
		return SchemaNameToTypeName(requestBodyName, nameNormalizer), nil
	}
...
func findSchemaNameByRefPath(refPath string, spec *openapi3.T, nameNormalizer NameNormalizer) (string, error) {
	...
	case "schemas":
		if schema, found := spec.Components.Schemas[propertyName]; found {
			return renameSchema(propertyName, schema, nameNormalizer)
		}
	case "parameters":
		if parameter, found := spec.Components.Parameters[propertyName]; found {
			return renameParameter(propertyName, parameter, nameNormalizer)
		}
	case "responses":
		if response, found := spec.Components.Responses[propertyName]; found {
			return renameResponse(propertyName, response, nameNormalizer)
		}
	case "requestBodies":
		if requestBody, found := spec.Components.RequestBodies[propertyName]; found {
			return renameRequestBody(propertyName, requestBody, nameNormalizer)
		}
	}
	return "", nil
}
