...
func (state *State) genParamArgs(params []ParameterDefinition) string {
	if len(params) == 0 {
		return ""
	}
	parts := make([]string, len(params))
	for i, p := range params {
		paramName := p.GoVariableName(state.NameNormalizer)
...
// parameter names:
// ", foo, bar, baz"
func (state *State) genParamNames(params []ParameterDefinition) string {
	if len(params) == 0 {
		return ""
	}
	parts := make([]string, len(params))
	for i, p := range params {
		parts[i] = p.GoVariableName(state.NameNormalizer)
	}
	return ", " + strings.Join(parts, ", ")
}
...
func (state *State) TemplateFunctions() template.FuncMap {
	return template.FuncMap{
		"genParamArgs":               state.genParamArgs,
		"genParamTypes":              genParamTypes,
		"genParamNames":              state.genParamNames,
